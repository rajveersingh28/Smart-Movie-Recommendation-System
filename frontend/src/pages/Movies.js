...
function Movies({ currentUserId }) {
    ...
    // Frontend real-time filter algorithm
    const filteredMovies = movies.filter(movie => 
        movie.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        movie.language_name.toLowerCase().includes(searchQuery.toLowerCase())
    );
            ...
            {filteredMovies.length === 0 ? (
                ...
                <div style={{
                    ...
                    {filteredMovies.map(movie => (
                        <MovieCard key={movie.movie_id} movie={movie} currentUserId={currentUserId} />
                    ))}
                </div>
            )}
        </div>
